...
def phi_x(x,d,s): 
    return sqrt(comb(d-1,s-1)) * (torch.cos(PI*x/2))**(d-s)\
                 * (torch.sin(PI*x/2))**(s-1) 

class MPS(nn.Module):
    """
    Matrix product state which converts input into a single output vector
    """
    def __init__(self, input_dim, output_dim, bond_dim, feature_dim=2,lFeat=2,
                 adaptive_mode=False, periodic_bc=False, parallel_eval=False,
                 label_site=None, path=None, init_std=1e-9, use_bias=True,
                 fixed_bias=True, cutoff=1e-10, merge_threshold=2000,tenetx=False):
        super().__init__()
...
#           x0 = torch.ones(tuple(input_data.shape[:2])).unsqueeze(2)
#           x = torch.cat((x0,input_data),dim=2)    
        ### Sinusoidal feature map
        if self.tenetx:
            if x.min() < 0:
                x = (x+1)/2
            x = torch.cat((x,1-x),dim=2)
        elif self.lFeat == 2 and not self.tenetx:
            x1 = torch.cos(x * PI/2)
            x2 = torch.sin(x * PI/2)        
#               x = torch.cat((x1,x2),dim=2)
            ### Interleave feature maps
            x = torch.stack((x1,x2),dim=3).view(x.shape[0],x.shape[1],-1)

#           x = [phi_x(input_data,self.lFeat,i) for i in range(1,(self.lFeat+1))]
#           x = torch.cat(x,dim=2)
